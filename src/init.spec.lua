...
return function()
    ...
    it("should return an identical string", function()
        expect(Fmt.format("Hello, world!")).to.equal("Hello, world!")
    end)

    it("should escape", function()
        expect(Fmt.format("{{}}")).to.equal("{}")
    end)

    it("should replace parameter", function()
        expect(Fmt.format("Hello, {}!", "Micah")).to.equal("Hello, Micah!")
    end)

    it("should format positional parameter", function()
        expect(Fmt.format("{1} {} {0} {}", 1, 2)).to.equal("2 1 1 2")
    end)

    it("should format extra positional parameter", function()
        expect(Fmt.format("{} {1}", 1, 2)).to.equal("1 2")
    end)
    ...
    it("should format named, regular, and positional parameters", function()
        expect(Fmt.format("Hello, {name}! How {} you {1}, {name}?", "are", "today", {
            name = "Micah";
        })).to.equal("Hello, Micah! How are you today, Micah?")
    end)
    ...
    it("should throw with extra parameters", function()
        expect(function()
            Fmt.format("Hello, {name}! How {} you {1}, {name}? {} {}", "are", "today", {
                name = "Micah";
            })
        end).to.throw("3 parameters found in template string, but there are 2 arguments.")
    end)
    ...
    it("should throw with extra arguments", function()
        expect(function()
            Fmt.format("Hello, {name}! How {} you {1}, {name}? {}", "are", "today", "yes", {
                name = "Micah";
            })
    ...
    it("should use debug form", function()
        ...
        expect((Fmt.format("{}", folder))).to.equal("Folder")
        expect((Fmt.format("{:?}", folder))).to.equal("Workspace.Folder")
    end)
    ...
    it("should add spaces to fit minimum width", function()
        expect(Fmt.format("{:5}", "x")).to.equal("x    ")
    end)

    it("should not add spaces when greater than minimum width", function()
        expect(Fmt.format("{:5}", "hello")).to.equal("hello")
    end)

    it("should handle sign", function()
        expect(Fmt.format("{positive} {positive:+} {negative:+} {zero:+} {message:+}", {
            positive = 5;
            negative = -5;
            zero = 0;
            message = "hello";
    ...
    it("should handle precision", function()
        expect(Fmt.format("{0:.5} {1:.3}", 5, 5.0001)).to.equal("5.00000 5.000")
    end)

    it("should handle leading zeros", function()
        expect(Fmt.format("{0:05} {1:02} {1:01}", 5, 42)).to.equal("00005 42 42")
    end)

    it("should handle array", function()
        local array = {1, 2, 3}

        expect(string.find(Fmt.format("{}", array), "table: 0x")).to.be.ok()
        expect(Fmt.format("{:?}", array)).to.equal("{[1] = 1, [2] = 2, [3] = 3}")
        expect(Fmt.format("{:#?}", array)).to.equal("{\n    [1] = 1,\n    [2] = 2,\n    [3] = 3\n}")
    end)
